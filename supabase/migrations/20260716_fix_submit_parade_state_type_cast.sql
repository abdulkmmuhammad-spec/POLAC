-- MIGRATION: 20260716_fix_submit_parade_state_type_cast.sql
-- DESCRIPTION: Fixes type mismatch in submit_parade_state RPC by safely casting p_course_number (TEXT) to INTEGER for parade_records.
...
BEGIN
    -- 1. Insert the parent parade record with safe INTEGER casting for course_number
    INSERT INTO public.parade_records (
        ...
        p_officer_id, p_officer_name, p_course_name, p_year_group, 
        NULLIF(regexp_replace(COALESCE(p_course_number, ''), '\D', '', 'g'), '')::INTEGER,
